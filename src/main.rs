...
use susi::config::*;
use susi::course_arranger::{
    calculate_arrangements, Arrangement, CategoryRequirements, CoursesWithCats,
};
...
async fn main() -> Result<()> {
    ...
    let electives_with_categories: HashSet<ElectiveCourse> =
        fetch_all_elective_pages_from_fmi_site().await?;
    ...
    electives_with_categories
        .iter()
        .for_each(|cat| corpus.add_text(&cat.name));

    // Now try to pair each susi course with it's elective counterpart from FMI's site
    let courses_with_electives_cats = electives_susi
        .into_iter()
        .filter_map(|susi_course| {
            electives_with_categories
                .iter()
                // First attempt an exact match
                .find(|fmi_elective_course| fmi_elective_course.name == susi_course.name)
                // If exact match not found, try fuzzy match with enough similarity
                .or_else(|| {
                    let closest_match = corpus
                        .search(
                            susi_course.name.as_str(),
                            susi::consts::FUZZY_SEARCH_SIMILARITY_THRESHOLD,
                        )
                        .into_iter()
                        .next();
                    ...
                    match closest_match {
                        None => {
                            println!(
                                "[WARNING] Course '{}' couldn't be found, so I'm ignoring it.\n",
                                susi_course.name
                            );
                        ...
                        Some(SearchResult {
                            ...
                            println!(
                                "Closest match is '{}' with similarity {}.\n",
                                name, similarity
                            );
        ...
        }) // Convert from Vec<Option<T>> to Vec<T> from Somes only
        .collect::<Vec<_>>();

    // Convert types for our arrangement calculator
    let courses_with_cats = courses_with_electives_cats
        .into_iter()
        .map(|(susi, fmi)| {
            (
                susi,
                fmi.categories.clone().into_iter().collect::<BTreeSet<_>>(),
            )
        })
        .collect::<Vec<_>>()
        .into_iter()
    ...
    // Convert types for our arrangement calculator
    let category_requirements = read_categories_config()?
        .into_iter()
        .map(|(k, v)| (k.into_iter().collect::<BTreeSet<_>>(), v))
        .collect::<BTreeMap<_, _>>();
    ...
    let (arrangement, (remaining_requirements, remaining_courses)) = result
        .iter()
...
fn display_arrangement_with_context(
    ...
    arrangement
        ...
        .for_each(|(cats, courses_with_cats)| {
            ...
            let course_with_cats_string = courses_with_cats
                .iter()
                .map(|(course, cats)| format!("  '{}' ({})", course.name, cats.iter().join("|")))
                .join("\n");
