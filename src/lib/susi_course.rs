...
impl Ord for Course {
    fn cmp(&self, other: &Self) -> Ordering {
        (self.year, self.exam_session, &self.name).cmp(&(
            other.year,
            other.exam_session,
            &other.name,
        ))
...
impl Course {
    pub fn from_susi_row(year: u16, semester: ExamSession, args: &Vec<String>) -> Result<Self> {
        ...
        let tutor = {
            let tutor_str = args.get(1).ok_or(anyhow!("Tutor not found."))?.clone();
        ...
        let is_elective = {
            let is_elective_str = args.get(2).ok_or(anyhow!("Elective flag not found."))?;

            is_elective_str == "Избираеми"
        };

        let is_passed = {
            let is_passed_str = args.get(3).ok_or(anyhow!("Passed flag not found."))?;

            is_passed_str == "да"
        };

        let grade = {
            let grade_str = args.get(4).ok_or(anyhow!("Grade not found."))?;
...
pub async fn fetch_susi_courses_raw_html() -> Result<String> {
    ...
    let req = client.post(consts::FMI_EXAMS_REPORT_URL).form(&form);
    let res = req.send().await?;
...
pub fn parse_susi_course_html(raw: &String) -> Result<Vec<Course>> {
    ...
    let my_table = html
        .select(&selector)
        .max_by_key(|tbody| tbody.children().count())
        .unwrap();
    ...
    for tr in year_trs {
        ...
        for (exam_session_index, exam_session_table) in exam_session_tables.enumerate() {
            ...
            let mut semester_courses = trs_fields
                .map(|fields| {
                    Course::from_susi_row(year, exam_session, &fields).context(format!(
                        "When parsing course from following fields: [{}]",
                        fields.iter().join(", ")
                    ))
                })
